-- Migration: Add damage reason, room 13/113 exclusion, and re-structure income report to sales_orders
-- Target: public.calculate_shift_totals, public.get_shift_closing_summary, public.get_income_report (both signatures)

-- Update 1: calculate_shift_totals (with room 13/113 exclusion)
...
-- Update 2: get_shift_closing_summary (with room 13/113 exclusion and courtesy reason)
...
-- Update 3: get_income_report (7 parameters) re-structured to sales_orders with room 13/113 exclusion
...
BEGIN
  ...
  IF p_report_type = 'shift' AND p_shift_id IS NOT NULL THEN
    ...
    -- Find sales_order_ids for this shift (exclude cancelled items and room 13/113)
    SELECT ARRAY_AGG(DISTINCT soid) INTO v_sales_order_ids
    FROM (
      SELECT soi.sales_order_id AS soid 
      FROM public.sales_order_items soi
      LEFT JOIN public.sales_orders so ON so.id = soi.sales_order_id
      LEFT JOIN public.room_stays rs ON rs.sales_order_id = so.id
      LEFT JOIN public.rooms rm ON rm.id = rs.room_id
      WHERE soi.shift_session_id = v_session_id 
        AND soi.sales_order_id IS NOT NULL 
        AND soi.is_cancelled IS NOT TRUE
        AND (rm.number IS NULL OR rm.number NOT IN ('13', '113'))
      UNION
      SELECT p.sales_order_id AS soid 
      FROM public.payments p
      LEFT JOIN public.sales_orders so ON so.id = p.sales_order_id
      LEFT JOIN public.room_stays rs ON rs.sales_order_id = so.id
      LEFT JOIN public.rooms rm ON rm.id = rs.room_id
      WHERE p.shift_session_id = v_session_id 
        AND p.sales_order_id IS NOT NULL 
        AND p.status NOT IN ('PENDIENTE', 'CANCELADO')
        AND (rm.number IS NULL OR rm.number NOT IN ('13', '113'))
      UNION
      SELECT rs.sales_order_id AS soid
      FROM public.room_stays rs
      LEFT JOIN public.rooms rm ON rm.id = rs.room_id
      WHERE rs.shift_session_id = v_session_id
        AND rs.sales_order_id IS NOT NULL
        AND (rm.number IS NULL OR rm.number NOT IN ('13', '113'))
    ) sub;
  END IF;
  ...
  filtered_orders AS (
    SELECT
      ...
      COALESCE(rs.id, so.id) AS stay_id,
      COALESCE(rs.check_in_at, so.created_at) AS check_in_at,
      rs.vehicle_plate,
      COALESCE(rs.status, so.status) AS stay_status,
      rm.number AS room_number,
      COALESCE(ev.first_name || ' ' || ev.last_name, '—') AS checkout_valet_name,
      COALESCE(e_in.first_name || ' ' || e_in.last_name, 
               COALESCE(e_so.first_name || ' ' || e_so.last_name, '—')) AS receptionist_name,
      COALESCE(sd.name, '—') AS shift_name
    FROM public.sales_orders so
    LEFT JOIN public.room_stays rs ON rs.sales_order_id = so.id
    LEFT JOIN public.rooms rm ON rm.id = rs.room_id
    LEFT JOIN public.employees ev ON ev.id = rs.checkout_valet_employee_id
    LEFT JOIN public.shift_sessions ss_in ON COALESCE(rs.shift_session_id, so.shift_session_id) = ss_in.id
    ...
    LEFT JOIN public.employees e_so ON e_so.auth_user_id = so.created_by
    WHERE
      -- Exclude rooms 13/113
      (rm.number IS NULL OR rm.number NOT IN ('13', '113'))
      -- Status filter
      AND (
        CASE WHEN p_status_filter = 'all' OR p_status_filter IS NULL
          THEN (rs.status IS NULL OR rs.status IN ('ACTIVA', 'FINALIZADA', 'CANCELADA'))
          ELSE (rs.status = p_status_filter OR (rs.status IS NULL AND p_status_filter = 'FINALIZADA' AND so.status = 'COMPLETED'))
        END
      )
      ...
      AND (
        CASE
          WHEN p_report_type = 'shift' AND v_sales_order_ids IS NOT NULL
            THEN so.id = ANY(v_sales_order_ids)
          WHEN p_report_type = 'dateRange' THEN
            (p_start_date IS NULL OR COALESCE(rs.check_in_at, so.created_at) >= p_start_date)
            AND (p_end_date IS NULL OR COALESCE(rs.check_in_at, so.created_at) <= (p_end_date + interval '1 day' - interval '1 millisecond'))
          ELSE true
        END
      )
    ORDER BY COALESCE(rs.check_in_at, so.created_at) ASC
  ),
  ...
  stay_entries AS (
    SELECT
      ...
      fo.room_number,
      fo.stay_status,
      fo.checkout_valet_name,
      fo.receptionist_name,
      fo.shift_name,
      -- Room price (exclude cancelled)
      COALESCE((
        SELECT SUM(soi.unit_price * soi.qty)
        FROM public.sales_order_items soi
        WHERE soi.sales_order_id = fo.sales_order_id
          AND soi.concept_type = 'ROOM_BASE'
          AND soi.is_cancelled IS NOT TRUE
          AND (v_session_id IS NULL OR soi.shift_session_id = v_session_id)
      ), 0) AS room_price,
      -- Extras (exclude cancelled and damages)
      COALESCE((
        SELECT SUM(soi.unit_price * soi.qty)
        FROM public.sales_order_items soi
        WHERE soi.sales_order_id = fo.sales_order_id
          AND soi.concept_type IN ('EXTRA_PERSON', 'EXTRA_HOUR', 'RENEWAL', 'PROMO_4H', 'ROOM_CHANGE_ADJUSTMENT')
          AND soi.is_cancelled IS NOT TRUE
          AND (v_session_id IS NULL OR soi.shift_session_id = v_session_id)
      ), 0) AS extra,
      ...
      -- Damages (exclude cancelled)
      COALESCE((
        SELECT SUM(soi.unit_price * soi.qty)
        FROM public.sales_order_items soi
        WHERE soi.sales_order_id = fo.sales_order_id
          AND soi.concept_type = 'DAMAGE_CHARGE'
          AND soi.is_cancelled IS NOT TRUE
          AND (v_session_id IS NULL OR soi.shift_session_id = v_session_id)
      ), 0) AS damage,
      ...
      (
        ...
        WHERE p2.sales_order_id = fo.sales_order_id
      ...
      (
        ...
        WHERE p3.sales_order_id = fo.sales_order_id
      ...
      (
        ...
        FROM (
          ...
          WHERE p4i.sales_order_id = fo.sales_order_id
  ...
  SELECT jsonb_build_object(
    'entries', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        ...
        'room_number', COALESCE(fe.room_number, '—'),
    ...
    'totals', (
      SELECT jsonb_build_object(
        'roomPrice', COALESCE(SUM(room_price), 0),
        'extra', COALESCE(SUM(extra), 0),
        'consumption', COALESCE(SUM(consumption), 0),
        'damages', COALESCE(SUM(damage), 0),
        'total', COALESCE(SUM(room_price + extra + consumption + damage), 0)
      )
      FROM filtered_entries
    ),
    'damage_items', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        'id', soi.id,
        'created_at', soi.created_at,
        'time', to_char(soi.created_at AT TIME ZONE 'America/Mexico_City', 'HH24:MI'),
        'room_number', COALESCE(rm.number, '—'),
        'reason', soi.courtesy_reason,
        'amount', soi.qty * soi.unit_price
      ) ORDER BY soi.created_at ASC)
      FROM public.sales_order_items soi
      LEFT JOIN public.sales_orders so ON so.id = soi.sales_order_id
      LEFT JOIN public.room_stays rs ON rs.sales_order_id = so.id
      LEFT JOIN public.rooms rm ON rm.id = rs.room_id
      WHERE soi.concept_type = 'DAMAGE_CHARGE'
        AND soi.is_cancelled IS NOT TRUE
        AND (rm.number IS NULL OR rm.number NOT IN ('13', '113'))
        AND (
          CASE
            WHEN p_report_type = 'shift' AND v_session_id IS NOT NULL THEN soi.shift_session_id = v_session_id
            WHEN p_report_type = 'dateRange' THEN
              (p_start_date IS NULL OR soi.created_at >= p_start_date)
              AND (p_end_date IS NULL OR soi.created_at <= (p_end_date + interval '1 day' - interval '1 millisecond'))
            ELSE true
          END
        )
    ), '[]'::jsonb),
...
-- Update 4: get_income_report (9 parameters - paginated) re-structured to sales_orders with room 13/113 exclusion
...
BEGIN
  ...
  INTO v_total_count, v_total_room_price, v_total_extra, v_total_consumption, v_total_damages
  ...
  SELECT jsonb_build_object(
    ...
    'totals', jsonb_build_object(
      ...
      'damages', v_total_damages,
      'total', v_total_room_price + v_total_extra + v_total_consumption + v_total_damages
    ),
    'entries', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        'no', fe.row_no,

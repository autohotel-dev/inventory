-- Migration to create cancel_reception_charge AND cancel_reception_item_v1
-- And to ensure the schema cache is reloaded
...
BEGIN
    ...
    IF FOUND THEN
        ...
        ELSIF v_item.concept_type = 'EXTRA_HOUR' THEN
            -- Cada item de hora extra tiene qty = 1
    ...
    IF FOUND THEN
        IF v_payment.amount <= v_item.total THEN
            DELETE FROM public.payments WHERE id = v_payment.id;
            v_payment_deleted := true;
        ELSE
            UPDATE public.payments SET amount = amount - v_item.total WHERE id = v_payment.id;
        END IF;
    END IF;

    -- 5. Update sales orders totals
    UPDATE public.sales_orders
    SET subtotal = GREATEST(0, subtotal - v_item.total),
        total = GREATEST(0, total - v_item.total),
        remaining_amount = GREATEST(0, remaining_amount - v_item.total)

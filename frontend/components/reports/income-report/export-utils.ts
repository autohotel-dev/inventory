...
<div class="header">
    ...
    <div class="meta">
        <b>${receptionistName}</b> &nbsp;|&nbsp; ${periodLabel} &nbsp;|&nbsp; ${entries.length} registros &nbsp;|&nbsp; Impreso: ${new Date().toLocaleString("es-MX")}
    </div>
</div>
<table>
    <thead>
...
export const handleHpPrint = async ({ entries, receptionistName, shiftInfo, startDate, endDate }: ExportParams) => {
    ...
    await sendPrintJob('income', printData, {
        endpoint: '/print/hp',
        successMsg: 'Reporte enviado a impresora HP',
        successDesc: `${entries.length} registros enviados a la impresora`,
    });
};

...
export function ShiftClosingModal({ session, onClose, onComplete }: ShiftClosingProps) {
  ...
  const loadPaymentSummary = async () => {
    ...
    try {
      ...
      // Extraemos con cuidado el UUID del empleado. Podria venir anidado o null
      ...
      if (shiftSalesOrderIds.length > 0) {
        const { data: ordersData } = await supabase
          ...
          .select(`
            ...
            room_stays(id, rooms(number, room_types(name))),
            sales_order_items(id, qty, unit_price, total, concept_type, is_paid, paid_at, payment_method, products(name, sku))
          `)
      ...
      console.log("DEBUG: All Shift Closing Payments Fetched:", payments);

      (payments || []).forEach((payment: any) => {
        console.log("DEBUG Eval:", payment.id, payment.amount, payment.payment_method, payment.status);

        // Ignorar pagos pendientes o envolturas "MIXTO"
      ...
      const enrichedPayments = (payments || []).map((payment: any) => {
        ...
        const itemsRawData = relatedItems.map((item: any) => {
          ...
          return { name: product?.name || conceptLabels[item.concept_type || "PRODUCT"] || "Item", qty: item.qty, unitPrice: item.unit_price, total: item.qty * item.unit_price };
        });
      ...
      setSummary({
        ...
        payments: enrichedPayments, salesOrders: salesOrders || [],
        expenses: expenses || [], total_expenses: totalExpenses,
        total_accrual_sales, accrual_items,
        unassigned_card_payments, unhandled_payment_methods
      });
    } catch (err) {
  ...
  // Guardar corte (simplificado — sin reconciliación)
    ...
    try {
      ...
      const { data: closing, error: closingError } = await supabase
        ...
        .insert({
          ...
          counted_cash: summary.total_cash, // Sin reconciliación, se asume correcto
          cash_difference: 0,
  ...
  const handlePrintClosing = async () => {
    ...
    try {
      const printData = {
        ...
        countedCash: summary.total_cash,
        cashDifference: 0,
        ...
        transactions: await Promise.all(summary.payments.map(async (payment: any) => {
          ...
          if (payment.sales_order_id && payment.itemsCount && payment.itemsCount > 0) {
            ...
            const conceptLabels: Record<string, string> = {
              ROOM_BASE: "Habitación", EXTRA_HOUR: "Hora Extra", EXTRA_PERSON: "Persona Extra",
              CONSUMPTION: "Consumo", PRODUCT: "Producto",
            };

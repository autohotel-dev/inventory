-- ================================================================
-- Enhanced Employee Risk Scoring - Granular Parameters
-- ================================================================
...
BEGIN
    ...
    LOOP
        ...
        -- 7. Manual adjustments (damage charges, other)
        ...
        -- 9. Cash payment ratio (higher = more risk)
        ...
        SELECT COALESCE(AVG(EXTRACT(EPOCH FROM (completed_at - started_at)) * 1000), 0) INTO v_avg_time_ms
        FROM operation_flows WHERE created_by = emp_rec.id AND completed_at IS NOT NULL;

        -- 12. Days since last anomaly
        ...
        END IF;
        IF v_courtesy_abuse > 0 THEN v_risk_factors := v_risk_factors || jsonb_build_object('factor', 'Abuso de cortesías', 'count', v_courtesy_abuse, 'points', CASE WHEN v_courtesy_abuse >= 5 THEN 15 WHEN v_courtesy_abuse >= 3 THEN 10 ELSE 5 END); END IF;

        -- Cash payment ratio (up to 10 pts) - higher cash = harder to track
        ...
        -- Recent anomaly (up to 5 pts) - more recent = higher risk
        ...
        out_avg_processing_time_ms := ROUND(v_avg_time_ms, 0);

-- Agrega el switch para auto-cobro de hora extra en system_config
ALTER TABLE system_config ADD COLUMN IF NOT EXISTS auto_charge_extra_hours BOOLEAN NOT NULL DEFAULT true;

COMMENT ON COLUMN system_config.auto_charge_extra_hours IS 'Si está habilitado, genera el cobro de la hora extra de forma automática';

-- Actualiza la función process_extra_hours_v2 para respetar la configuración
...
DECLARE
  ...
  v_auto_charge BOOLEAN;
BEGIN
  -- 0. Verificar si el autocobro está habilitado
  SELECT auto_charge_extra_hours INTO v_auto_charge FROM system_config LIMIT 1;
  IF NOT v_auto_charge THEN
    RETURN jsonb_build_object('success', true, 'hours_added', 0, 'message', 'Autocobro desactivado en configuración');
  END IF;
